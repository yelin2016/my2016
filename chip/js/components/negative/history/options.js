...
var Options = React.createClass({
	...
	componentDidMount: function () {
		...
		// 获取分级数据
		this.props.actions.getHistoryWaringLevel ();
		// 获取网站类型数据
		this.props.actions.getHistorySourceTypeId();
		// 获取网站数据
		...
		// 获取列表数据
		this.props.actions.getHistoryData(this.getParam(this.props));
	},
	componentWillReceiveProps: function(nextProps) {
	  if (this.props.refresh==false && nextProps.refresh == true){
	  	 this.props.actions.getHistoryData(this.getParam(nextProps));
  	}
	},
	...
	getParam: function (props) {
      ...
      if (condition[name] !== '') {
		...
		var param = {
			...
			terminateWay: condition.terminateWay,
	...
	query: function () { 
		this.props.actions.history_query({
			...
			terminateWay: this.state.terminateWay,
	...
	download:function(){
		...
		var url = ApiDomain + "/opinion/rest/negative/getNegativeHistoryDownload?";
	...
	render: function () {
		...
		return (
			...
			<div className='options_line'>
				...
				<Options_select handlechg={this.inputsChg} keys='objectContent' title='预警内容'/>
				<div className='options_select'>
					     <div className='options_select_title'>
					     	<span>网站类型:</span>
					     </div>
				...
				<div className='options_select'>
					 	...
					 	<div  className='options_select_input'>
					 	    <DropList 
								curActive={this.state.curDropActive}
								curoption={this.state.terminateWay}
								propKey="terminateWay" 
								chgOption={this.chgOption} 
								chgActive={this.chgActive}
								dropData={this.props.stateData.TerminateWaydata}
								nodefault={true} />
					 	</div>	 
			    </div>
			...
			<div className='options_line'>
				...
				<Function_button  value='下载表格' click={this.download} color='#5983b2'/>
			</div>
			</div>
		);
	}
